...
def test_recover_endpoint_returns_202_then_succeeded(db_engine, migrated_db_url, tmp_path, _teardown_compose_projects):
    ...
    install_response = client.post(
        ...
        json={"configuration": {"APP_PORT": {"value": 19510}}, "requested_by": "operator:test"},
    )
    wait_for_terminal_operation(db_engine, install_response.json()["data"]["operation_id"])

    target = _import_via_api(client, "update-verify-fails")
    ...
    update_op = update_response.json()["data"]["operation_id"]
    failed = wait_for_terminal_operation(db_engine, update_op, timeout=60)
    assert failed["status"] == "FAILED"

    backup_id = client.get(f"/api/v1/applications/{application_id}/backups").json()["data"]["items"][0]["backup_id"]

    recover_response = client.post(
        f"/api/v1/applications/{application_id}/recover",
        json={"failed_operation_id": update_op, "backup_id": backup_id, "requested_by": "operator:test"},
    )
    assert recover_response.status_code == 202
    recover_op = recover_response.json()["data"]["operation_id"]
    final = wait_for_terminal_operation(db_engine, recover_op, timeout=60)
    assert final["status"] == "SUCCEEDED"

    active = client.get(f"/api/v1/applications/{application_id}/active-deployment").json()["data"]
    assert active["release_id"] == source["release_id"]
